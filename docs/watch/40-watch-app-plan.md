...
Státusz: **Android (Wear OS) — F0–F4 megvalósítva és emulátoron végigtesztelve (2026-07-16); az F4B design-adósság (12. fejezet) Wear OS oldalon nagyrészt lefejlesztve és fut Wear OS emulátoron ellenőrizve (2026-07-17, lásd 7.5.9) — a teljes brand-styling (B6) és a legtöbb funkcionális elem (B1–B5, B11–B13) kész; hátravan még az ENDING/SUMMARY fázispár (A5) és az ambient-variáns (D0.3). iOS (Apple Watch) — F0–F4 megvalósítva (2026-07-16), build-ellenőrizve (LifeyWatch target + teljes Runner workspace zöld); watchOS-szimulátoros/fizikai eszközös manuális teszt még hátravan, lásd 11.5. F4B-állás (2026-07-17): Android B1–B6/B12/B13 kész (`c2c42b7` + „Android watch F4 + design”); iOS-en B1–B10, beleértve B6-ot is (minden 12.1–12.2-es tétel), kész (ugyanaznap, lásd 12.1); a telefon-oldali elemek (B14–B15, közös Dart-kód + mindkét natív híd) szintén kész (2026-07-17, lásd 12.4) — csak a Wear-oldali ENDING/SUMMARY (A5) és az ambient-variáns (D0.3) maradt hátra, mindkét platformon.**
...
| F5b — (v2) Reps/súly állítása a watchról | ⏸ Terv kész, minden döntés lezárva ([48-doc](48-watch-f5b-set-adjust-plan.md)) — implementáció nem kezdődött el | ⏸ ua. |
...
**Amit ez NEM fed le** (F4B-ből még hátravan Androidon): A5 (ENDING/SUMMARY Wear-fázisok — nincs hozzá canvas-frame sem), ambient/dimmed-variáns (D0.3). A telefon-oldali „Measuring”-pill és ⌚-badge (B14/B15) ennek a (Android-specifikus) munkamenetnek sosem volt része — **azóta, 2026-07-17-én, egy külön munkamenetben elkészültek** (lásd 12.4). iOS oldalon a teljes F4B (B1–B10 iOS-fele) érintetlen.
...
**Frissítés (2026-07-17, még aznap)**: Wear OS oldalon a legtöbb lista alábbi tétele (B1–B6, B11–B13) azóta elkészült — lásd 7.5.9. A táblázatok alább **változatlanul a felmérés eredeti állapotát** mutatják referenciaként (mit írt le a canvas mint hiányt), az egyes sorok végén ✅-jelöléssel és 7.5.9-hivatkozással jelezve, mi készült el azóta. iOS oldalon (12.2) semmi nem változott — az a lista továbbra is pontosan leírja a hátralévő munkát. A telefon-oldalon (12.4) B14–B15 is elkészült, ugyanennek a napnak egy külön munkamenetében — lásd a 12.4 sorainak ✅-jelölését.

A frame-hivatkozások a canvas számozását követik (Apple Watch 01–07, Wear OS 01–06, telefon A–C).

**Állapot-frissítés (2026-07-17, „Android watch f4b”/„Android watch F4 + design” + iOS B1–B10 munkamenet)**: az Android-oldal a `c2c42b7` és a rákövetkező „Android watch F4 + design” commitban leszállította B1–B5-öt, B11–B13-at és a teljes B6-styling-ot; iOS-en egy külön munkamenetben **B1–B10, azaz a teljes 12.1–12.2-es lista, B6-tal együtt** elkészült (`WorkoutManager.swift`/`PhoneConnector.swift`/`ContentView.swift`/`ActiveWorkoutView.swift`/`IdleView.swift`/`EndingView.swift`/`SummaryView.swift`/`HealthDeniedView.swift`/új `DynamicSizing.swift`/új `Theme/LifeyColors.swift`/új `Theme/LifeyShapes.swift`), a shared Dart-oldali `restTotalSeconds`/`restRemainingSeconds` mező pedig már mindkét platform hídján átmegy (ez volt a P3-tétel, az Android-commitban készült el, közös kódként iOS-t is kiszolgálja). Az iOS `LifeyColors.swift` value-for-value ugyanazokat a hexeket használja, mint az Android `LifeyColors.kt` (a canvas Apple Watch-frame-jeivel — AW 01–07 — összevetve, nem csak az Android-kóddal). A Wear-oldali ENDING/SUMMARY (A5) és ambient-variáns (D0.3) mindkét platformon még hátravan (lásd a 42-es doc D1–D2 fejezetét) — ezek a maradék tételek a 12. fejezet listájából.

**Állapot-frissítés (2026-07-17, B14+B15 munkamenet)**: a telefon-oldali B14–B15 (12.4) elkészült. B14 („Measuring” ⌚-pill): mivel a `startedOnWatch` natív jelzést a felmérés idején valójában **semelyik platform sem küldte** (a Dart-oldali `'startedOnWatch'` string csak dokumentált, de sosem előálló eseménytípus volt), ehhez mindkét natív watch-oldalt is bővíteni kellett — iOS: `WorkoutManager.swift` egyszeri (nem minden state-syncre ismétlődő) `PhoneConnector.sendStartedOnWatch`-hívással jelez, amint `phase == .active` és `sessionClientId` is ismert (a kettő sorrendje nem garantált, lásd a metódus doc-kommentjét); Android: `ExerciseService.kt` a `SessionStateHolder.onExerciseActive` után hívja a `SummarySender.sendStartedOnWatch`-ot. Mindkét telefon-oldali natív híd (`WatchBridge.swift`/`WatchBridge.kt`) relézi Dart felé; a `watch_workout_service.dart` új `WatchStartedOnWatch`/`WatchReachabilityChanged` típusokat dekódol (utóbbi az iOS-en már meglévő, eddig fel nem használt `reachabilityChanged` jelzést köti be — Androidon nincs neki natív forrása, így ott sosem tüzel, ami best-effort módon ártalmatlan). A `LogSessionScreen` egy `_measuringOnWatch` flaggel vezérelt ⌚-pillt renderel a fejlécben, a meglévő elapsed-timer/HR-pillek pontos mintáját követve — megjelenik `startedOnWatch`-ra, eltűnik reachability-vesztésnél vagy a session végén (`_finishedAt != null`). B15 (⌚ „Watch” badge): a D0.4 döntésnek megfelelően nem lett új mező — a `WorkoutSession.fromAppleHealth` getter átnevezve `enrichedFromWatch`-ra (`workout_session.dart`), a `sessions_tab.dart` session-kártya-badge ikonja `Icons.favorite`-ról `Icons.watch`-ra váltott, új `enrichedFromWatchTooltip` l10n-kulccsal (EN: „Measured by watch”, HU: „Órán mérve”) a régi `importedFromHealthTooltip` helyett. Ellenőrzés: `flutter analyze` és a `test/features/workouts` teljes suite-ja (50 teszt) zöld; Android `:wear:compileDebugKotlin`/`:app:compileDebugKotlin` zöld; iOS `LifeyWatch`-forrásfák önálló `swiftc -typecheck`-je és a teljes `Runner` workspace-build (fizikai eszköz céltávval, code signing nélkül) is zöld — a watchOS-szimulátoros/fizikai manuális teszt (11.5) erre a két tételre sem futott még le.
...
| B2 | **Rest-end „GO” vizuális pillanat**: a 0-ra érésnél a haptika mellé 1–2 mp-es vizuális flash/átmenet | prompt §3.4 (a canvas csak „snaps back to metrics”-et említ — lásd 12.5) | **✅ Kész mindkét platformon.** Android: `ActiveWorkoutScreen.kt`'s `GoFlash` (`c2c42b7`). iOS: `ActiveWorkoutView.swift`'s `GoFlashView` + `runGoFlashCycle()` — `.task(id: workoutManager.restDeadlineUptime)` mirrorozza az Android `LaunchedEffect`-jét (a task cancel-elődik/újraindul, ha a rest skip/replace miatt új deadline érkezik, mielőtt a régi natural-zero-ra érne — így nem villan fel hamisan). A haptika (`WorkoutManager.scheduleRestHaptic`) továbbra is független ettől. |
| B3 | **Pause/Resume** (csak a szenzor-sessiont pauzálja, a telefon-session időzítését nem) | AW 04, Wear 03 | **✅ Kész mindkét platformon.** Android: `ExerciseService.pause`/`.resume`, `SessionStateHolder.onPausedChanged` (`c2c42b7`). iOS: `WorkoutManager.pause()`/`.resume()` a `HKWorkoutSession`-en; az `isPaused` a `HKWorkoutSessionDelegate`'s `didChangeTo toState:` callbackből származik (nem közvetlenül a hívás helyén állítjuk be), mirrorozva az Android `ExerciseUpdateCallback.exerciseStateInfo.state.isPaused`-ját — mindkettő a rendszer (Health Services / HealthKit) tényleges állapotváltását tükrözi, nem az UI-optimista feltételezést. A gomb+„Paused” caption `ActiveWorkoutView.swift`-ben. |
| B4 | **Dynamic sizing**: százalék-alapú paddingek és metrika-típusskála (SwiftUI `ViewThatFits`/scaled metrics, Compose `BoxWithConstraints`-frakciók), 41 mm / 1.2″ ellenőrzéssel | canvas „Dynamic sizing” sor | **✅ Kész mindkét platformon.** Android: `DynamicSizing.kt`, `isCompactScreen` (`c2c42b7`). iOS: új `DynamicSizing.swift` (`screenPaddingFraction`, `compactScreenWidth = 190pt`, `isCompact(width:)`) — `ActiveWorkoutView`/`IdleView` `GeometryReader`-be csomagolva (a tervezett `ViewThatFits` helyett, ami itt nem illett: nem méret szerinti *választás* kellett, hanem folytonos, %-alapú skálázás), a rest-ring mérete és a hero/caption betűméret is a dial méretéből számolt frakció, nem fix pt. A `project.pbxproj`-ba kézzel bekötve, `plutil -lint` + `xcodebuild -target LifeyWatch` zöld. |
| B5 | **Idle brand-moment**: levél/eco jel + „Lifey” wordmark az üres képernyőn | AW 01, Wear 01; prompt §3.1 | **✅ Kész mindkét platformon.** Android: `IdleScreen.kt`'s `LeafMark` (`c2c42b7`). iOS: `IdleView.swift`'s `LeafMark`/`LeafShape` — SwiftUI `Shape` + két cubic-Bézier-görbe és egy közép-„erezet” téglalap, 1:1 az Android Canvas `Path.cubicTo`-rajzával, a dial méretéhez képest %-osan skálázva (B4-konzisztens, nem fix pt). |
| B6 | **Teljes brand-styling** (átfogó): warm-black rétegzett felületek, moss-olive primary, metric-accent színek (♥ canvas `#D97F7F` — a prompt §2.4 saját `#C46A6A`-jától eltérően, lásd `LifeyColors.kt`/`LifeyColors.swift` megjegyzése; kcal `#E0915A`), „STRENGTH” fejléc-chip, szett-számláló pill/dot, tabular numerálok, radius-skála | minden frame; prompt §1–2 | **✅ Kész mindkét platformon.** Android (Wear OS): `LifeyColors.kt`/`LifeyShapes.kt`/`LifeyTheme.kt` (`material-icons-extended`), lásd 7.5.9 — a szett-számláló Androidon szöveg maradt, dot nélkül (a canvas Wear-sora sem mutat dotot, csak az Apple Watch-sor AW02, tehát ez nem elmaradás). iOS: új `Theme/LifeyColors.swift` (value-for-value ugyanazok a hexek, mint `LifeyColors.kt`, `Color(hex:)` inittel) + `Theme/LifeyShapes.swift` (radius-skála) — a canvas Apple Watch-frame-jeit (AW 01–07) követi pixel-közeli hűséggel: `IdleView` levél-badge SF Symbol `leaf.fill`-lel (nem kézzel rajzolt Path, mint korábban/Androidon — a natív SF Symbol pontosabb egyezés), `ActiveWorkoutView` STRENGTH/REST fejléc-chip + set-számláló **dot-sor** (AW02-ben van, Androidon nincs — itt bepótolva, mert a user kifejezetten a canvas-egyezést kérte), AW04 két nagy kör End/Pause gombbal (Android inkább teljes szélességű pill-chipeket használ — ez a natív platform-idióma eltérés, a canvas iOS-frame-je viszont köröket mutat, azt követtem), `EndingView`/`SummaryView`/`HealthDeniedView` a saját canvas-frame-jük (AW05/06/07) színei+ikonjai+pontos copy-ja szerint. `.monospacedDigit()` a tabular numerálokhoz. |

### 12.2 iOS-specifikus hiányok

| # | Hiányzó funkció | Design-forrás | Mai állapot a kódban |
|---|---|---|---|
| B7 | **Lapozós szerkezet (TabView), külön controls-lap** End+Pause gombbal, Apple Workout-minta | AW 02+04; terv §4.4 (eredetileg is 2 lap volt tervben) | **✅ Kész.** `ActiveWorkoutView.swift`-ben `TabView` + `.tabViewStyle(.page)` két lappal: `MetricsPage` (eltelt/pihenő idő, gyakorlat/szett, HR/kcal) és `ControlsPage` (Pause/Resume + End) — a korábbi egy-képernyős, inline End gombos elrendezés helyett. A `runGoFlashCycle`/GO-flash a felső szinten maradt (`ActiveWorkoutView`, nem `MetricsPage`), hogy bármelyik lapon is legyen épp a user, a flash átfedje a képernyőt. |
| B8 | **„End requested — waiting for phone” képernyő** („Finish on your iPhone / Rate your effort…”) az End megnyomása után, amíg a telefon `end` parancsa vissza nem ér | AW 05 | **✅ Kész.** `WorkoutPhase` új `.ending` esettel bővült; `requestEnd()` erre vált (a szenzorok tovább mérnek, a `HKWorkoutSession` érintetlen), `ContentView` erre az új `Views/EndingView.swift`-et mutatja („Finish on your iPhone” / „Rate your effort…”, `ending_title`/`ending_subtitle` kulcsok HU/EN-ben — nincs Android-referencia ehhez, mert a Wear-oldali ENDING/SUMMARY még nincs megtervezve, lásd D1.3/A5 a 42-es docban). A `finishAndSendSummary()` guard és a `desiredPhase: "ended"` kézbesítési-garancia fallback (`PhoneConnector`) mindkettő `.active`-ról **és** `.ending`-ről is elfogadja a valódi lezárást, mert az utóbbi (unreachable watch, sosem küldött endRequested-et) `.active`-ban futhat be. |
| B9 | **„Workout saved” összegző képernyő**: teljes idő, átlag bpm, kcal, „Saved to Health” jelzés, ~6 mp után auto-dismiss → Idle | AW 06 | **✅ Kész.** `WorkoutPhase` új `.summary(WorkoutSummaryData)` esettel bővült (asszociált érték, nem külön published mező — csak a fázissal együtt létezik). `finishAndSendSummary()` a lezárás után ebbe vált (`totalDuration`/`averageHeartRate`/`activeCalories`/`savedToHealth` — utóbbi azt jelzi, hogy jött-e valódi `healthWorkoutId`, nem csak hogy volt-e HR/kcal adat), `ContentView` erre az új `Views/SummaryView.swift`-et mutatja, `scheduleSummaryAutoDismiss()` egy `Task`-kal 6 mp után `.idle`-re vált. Nincs Android-referencia (a Wear-oldali SUMMARY még nincs megtervezve, lásd D1.3/A5 a 42-es docban), a szöveg a doc §12.1 B9 idézetéből jön (`summary_title`/`summary_saved_to_health` kulcsok HU/EN-ben). |
| B10 | **HealthKit-engedély-hiba képernyő** („Allow Health access” + „Review access” gomb) | AW 07 | **✅ Kész.** `WorkoutPhase` új `.healthDenied` esettel bővült; `start(configuration:)` a `requestAuthorizationIfNeeded()` után ellenőrzi a `workoutType()` (share-típus, tehát lekérdezhető, szemben a read-típusokkal) `authorizationStatus`-át — `.sharingDenied` esetén ide vált, a session el sem indul (a korábbi, §9-ben dokumentált „csendben Idle-be esik” viselkedés helyett). `ContentView` erre az új `Views/HealthDeniedView.swift`-et mutatja, a „Review access” gomb `WorkoutManager.dismissError()`-t hív, ami vissza `.idle`-be vált (watchOS-en nincs publikus API a Settings-hez való deep-linkeléshez, ez a 42-es doc D1.2/W5-ös „minimum: instrukció + dismiss” döntése). A „másik app fut” hiba (`startRejected`) változatlan maradt — annak nincs iOS-en dedikált képernyője a §12.1 szerint, csak a telefon-oldali snackbar. |
...
| B11 | **`ScalingLazyColumn`-szerkezet**: metrikák felül, lejjebb görgetve a controls-szekció (End workout + Pause), jobb oldali scroll-jelzővel | Wear 02+03 | **✅ Részben kész, más komponenssel (`c2c42b7`)**: `ActiveWorkoutScreen.kt` egy scrollozható `Column`-t használ `ScalingLazyColumn` helyett — az utóbbi alapértelmezett auto-centerezése (a 2. elemre, azaz a controls-sorra középre) egy valódi bugot okozott (a metrikák teteje induláskor a képernyőn kívül esett), ez dokumentált eltérés, nem hiány. |
| B12 | **„Workout already running” hibaképernyő az órán** (OK gombbal) | Wear 05; terv §5.3 („a watch UI hibát mutat”) | **✅ Kész** (`ErrorScreen.kt`, `SessionStateHolder.SessionPhase.ERROR`, `c2c42b7`) |
| B13 | **Degradált HR-állapot**: „––” placeholder (muted) + „Heart rate off — allow sensors” chip, ami az engedély-sheetet nyitja | Wear 06; prompt §3.6 | **✅ Kész** (`ActiveWorkoutScreen.kt`'s `hasHeartRatePermission`-ág + `CompactChip`, `c2c42b7`) |

### 12.4 Telefon-oldali hiányok (Flutter)

| # | Hiányzó funkció | Design-forrás | Mai állapot a kódban |
|---|---|---|---|
| B14 | **„Measuring” pill** a log-képernyő fejlécében, a `workoutStartedOnWatch` eseményre | telefon B | **✅ Kész (2026-07-17).** A felmérés idején tévesen dokumentáltként szerepelt „a Dart-hídon átjön” — valójában **egyik natív platform sem küldte** ezt az eseményt, azt is le kellett fejleszteni: iOS `WorkoutManager.swift` egyszeri `PhoneConnector.sendStartedOnWatch`-hívással jelez, amint `phase == .active` és `sessionClientId` is ismert; Android `ExerciseService.kt` a `SessionStateHolder.onExerciseActive` után hívja a `SummarySender.sendStartedOnWatch`-ot; mindkettőt a telefon-oldali natív híd (`WatchBridge.swift`/`WatchBridge.kt`) relézi Dart felé. `watch_workout_service.dart` új `WatchStartedOnWatch`/`WatchReachabilityChanged` típusokra dekódolja. `LogSessionScreen`'s `_measuringOnWatch` flag vezérli a fejléc-pillt (a meglévő elapsed/HR-pill mintáját követve) — megjelenik `startedOnWatch`-ra, eltűnik reachability-vesztésnél vagy session-végén. |
| B15 | **⌚ „Watch” badge** a session-kártyán a régi „Health” badge helyett, amikor a metrikák watch-summaryból jöttek | telefon C; §6.3 („később külön ⌚ badge”) | **✅ Kész (2026-07-17), a D0.4 döntés szerint (nincs új mező).** `WorkoutSession.fromAppleHealth` getter átnevezve `enrichedFromWatch`-ra (`workout_session.dart`); `sessions_tab.dart` badge-ikonja `Icons.favorite` → `Icons.watch`, új `enrichedFromWatchTooltip` l10n-kulccsal (a régi `importedFromHealthTooltip` helyett). |

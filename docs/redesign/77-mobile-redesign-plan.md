...
*As built:* the wizard's frame in `onboarding_screen.dart` matches the canvas: a **six-segment progress bar** (5 dp segments, one per step, the reached ones in the primary colour; five segments where there is no health store, i.e. off Android / iOS) with "Skip" beside it, **"Step 4 of 6"** under it on *every* step — including welcome and the Health step — and the step title as the shared **`ScreenHeading`** (36/800, moved with the brand tile to `shared/widgets/ds/screen_heading.dart` as `ScreenHeading` / `BrandTile`, so login and onboarding use one). The button hierarchy: "Get started" / "Next" is the full-width 56 dp primary button, "Back" a quiet 56 dp button beside it; on the plan step "Apply these goals" is full width with "Back" and a text-weight "Not now" below it. Welcome and Health are left-aligned like the sign-in screen (brand tile + heading + message; the Health step's Enable / Not now sit at the foot). **"Not now" on the Health step leaves the connection off** — it only calls the exit to the dashboard, nothing is requested or stored (unchanged behaviour, now stated in the code; the desktop test host has no health step, so this is checked on the emulator in the R5 review). Back / Skip / validation behave as before. ARB: `onboardingStepOfLabel`. 6 widget tests (`onboarding_frame_test`: step label and one segment per step, primary button, Back, refusal without an answer, no overflow at 360 dp × 1.3 in EN / HU, dark and light).
...
*As built:* the step asks **"How active are you?"** (`onboardingLifestyleQuestion`; the old "Lifestyle & goal" title stays in Settings) and the five answers are a **single column of `RadioOptionRow`** cards (new in `widgets/option_card.dart`): a 44 dp icon holder (neutral, primary-tinted when chosen), title 16/700, the description in full — no `maxLines`, so the Hungarian copy wraps instead of clipping — and a 26 dp radio mark on the right (an outline, or a filled primary circle with a check); the chosen row takes the primary tint and a 1.5 dp primary ring. Spoken as one mutually exclusive group ("Moderate, Exercise 3–5 days a week", checked). Under it **"PRIMARY GOAL"** (`SectionLabel`) and the three goal tiles — `OptionCard` restyled to the canvas (icon over label, card surface, hairline, primary tint + ring when chosen; it lost its `description`, which only the removed grid used). The descriptions now read as on the canvas ("Exercise 1–3 days a week", "Hard exercise and a physical job", HU "Mozgás heti 1–3 napon" …). Body & goals (Settings) gets the same radio list. The optional target weight, validation and the values sent are unchanged. ARB: `onboardingLifestyleQuestion`; the four `onboardingActivityDescription*` texts. 3 widget tests (`onboarding_steps_test`: five rows stacked in one column at equal width, exactly one row marked and Next refused without a goal, long HU answers wrap at 360 dp × 1.3).
...
*As built:* **the plan step** is the canvas' calorie hero: the subtitle "Built for building muscle at a moderate activity level." (`onboardingSuggestedSubtitle`, one ICU `select` message per language so the article and the Hungarian endings stay right), a `LifeyCard.hero` with the calorie flame and "Daily calories", the number at **60 px** (`MetricValue`, fixed under text scaling), three neutral chips — **BMR 1,384 · TDEE 2,145 · +10% surplus** ("−15% deficit" / "At maintenance" for the other two cases) — a `RatioBar` of the three macros, and under it Protein / Carbs / Fat each in its own colour with the grams at 26 px and the share of energy. Below the hero the **water tile** ("2.6 L per day") and the footnote "You can change these anytime in Settings → Body & goals." (HU: "Beállítások → Testadatok és célok" — the real screen name). **No maths changed:** every number is what `suggestGoals` returned; the two derived figures are display-only and unit-tested (`domain/plan_shares.dart`, 6 tests): the surplus is `calories / tdee − 1` rounded, and the shares are of the energy the three macros add up to (4 / 4 / 9 kcal per gram), rounded by the largest remainder so they always total 100 (129 g / 313 g / 66 g → 22 / 53 / 25, as on the canvas). The old two-column tile grid and the "based on a BMR of …" sentence are gone (`onboardingSuggestedFromMessage` removed). **`confirm_save_details_dialog.dart`** lost its ten colour literals: surface, radius and title come from the dialog theme (the same as the logout dialog), the changed-field rows and the recalculated-goal chips use the nested surface and the text styles, Cancel is the quiet button and Save the primary one. **`onboarding_edit_screen.dart`**: `AppBar` → `LifeySubpageHeader`, the fields drop their hard-coded outline borders (the filled theme applies), the labels are `SectionLabel`s, the date of birth is a `DateRow` card (also used in the wizard) and Save is a 56 dp primary. The goal is now called "Build muscle" (canvas). ARB: `onboardingSuggestedSubtitle`, `onboardingDailyCaloriesLabel`, `onboardingChipBmr/Tdee/Surplus/Deficit/Maintenance`, `onboardingWaterPerDay`, `onboardingChangeLaterMessage` (new text). Tests: the plan step of the wizard against a fake repository (hero numbers, chips, shares, footnote, button hierarchy) and `plan_shares_test`.
...
*As built:* **the thread header** is `LifeySubpageHeader` (the chat's own `AppBar` is gone): the round back button, the peer's tinted monogram avatar (44 dp; `ChatAvatar` now takes its tint from the person, like the list), the name and — as a subtitle — **the person's role, "Your trainer" / "Your client"** (`chatPeerRoleTrainer/Client`); search as a `HeaderIconButton`, the trainer's "view their data" button as before, and **mute / unmute moved into a ⋮ menu** (`chatMoreTooltip`). **Deviation: no "· online".** The canvas says "Your trainer · online", but presence is only tracked to decide whether a push goes out (§5.1) and is never reported back to the client, so the word would be invented; the subtitle states the role and nothing more. **Bubbles:** own messages are the primary colour with `onPrimary` text (the tinted-olive look is gone), the peer's the card surface with a hairline; 15/500 text (`bodyMedium`), radius 22 with the flattened 8 dp corner at the end of a run, up to 80 % of the width; **the time and the receipt sit under the bubble** (13 px, tertiary text; the ticks keep their distinct shapes, "read" is the primary colour), only under the last bubble of a run. The avatar column beside the peer's bubbles is gone (the header carries the person). The deleted-message tombstone and the long-press sheet are unchanged. **Day divider:** a quiet pill on the control surface. **Composer:** a 48 dp pill (card fill, hairline, 15 px text) between a 48 dp square picture button (`SquareIconButton`) and a round 48 dp send button (primary when there is something to send, the control surface when not, an up arrow), on the page colour under a hairline; the archived-thread notice and the typing band use the same tokens (the band's height now follows the text size so ×1.3 never clips it). **Deviation: no blur scrim** — nothing scrolls under the bar (the list ends above it), so there is nothing to blur; the hairline separates them. The offline strip, skeleton and load-error states are on tokens (`ErrorView`). Nothing about sending, retrying, unsent handling or the stream changed; the three Windows attachment failures in `chat_repository_test` are the known ones. Tests: `message_bubble_test` (+3: own = primary / on-primary, peer = card text, time under the bubble), new `chat_composer_test` (48 dp field and round 48 dp send, send gating, no overflow at 360 dp × 1.3 in EN / HU, dark and light, archived notice, day divider).
...
*As built:* the logic is a small service, `core/sync/logout_preflight.dart` (`LogoutPreflight`, behind `logoutPreflightProvider`), taking the outbox count, an "online?" check and one `SyncEngine.sync()` as functions so the rules are unit-tested without a database or network (`logout_preflight_test`, 9 tests). `plan()` counts the queued changes (every row of the outbox: pending, failed, in flight) and asks the connectivity check → `LogoutPlan` (`isClean` / `willUpload` / `willLose`). `flush()`: nothing queued → no network round trip; **online → one drain of the outbox, bounded by a 10 s timeout** (a bad connection cannot hold the logout hostage; a slow or failing upload never throws out of it); offline → nothing is attempted; either way it returns how many changes are still queued. `AuthController.logout({flush = true})` runs the flush **before** its existing sequence, which is untouched — token revoke, `clearAllData`, every per-account preference sweep, `LocalDataGuard.release` — so the cross-account clean-up (#43) holds whatever the flush found. Settings → Log out now asks `plan()` first and the dialog picks its body: **clean** "Everything is saved to your account. Your data on this phone is removed."; **online with queued changes** "3 unsynced changes will be uploaded first. Your data on this phone is removed." (the plural is ICU; the canvas' "from the last 2 minutes" is not used — the outbox has no such window, so the number would be invented); **offline with queued changes** the warning "4 changes couldn't be uploaded and will be lost. You're offline. …" in the heart colour. After Confirm, when there is something to upload, a small non-dismissible progress dialog ("Uploading your changes…") shows while the flush runs, then `logout(flush: false)`. ARB: `logOutDialogMessage` (new text), `logOutDialogMessageUpload`, `logOutDialogMessageLoss`, `logOutUploadingMessage`. Tests: 9 for the preflight (plans for clean / online / offline, one drain when online, no attempt offline or when nothing is queued, timeout, failure, refused changes still counted) and 3 new `logout_dialog_test` cases (the three bodies incl. the singular). Not covered by an automated test: the wipe order inside `AuthController.logout` itself (it needs the whole account graph); it is unchanged code and is exercised in the R5 emulator review's logout.
...
*As built:* `settings_screen.dart` went from 1 637 to ~700 lines — a `CustomScrollView` under the large-title `LifeyHeader` ("Settings"; new `onBack` option puts the round back button beside the title, because Settings is a pushed screen, and header_test covers it) — and its parts live in `settings/presentation/widgets/`: `settings_kit.dart` (`SettingsRow` — the DS `ListRow` with a neutral 40 dp icon holder —, `SettingsValue` "System ›", `InlinePillSegment`, `SettingsSwitch`), `settings_profile_card.dart`, `settings_goals.dart` (`DailyGoalTiles`, `showGoalEditSheet`), `settings_integrations.dart`, `settings_my_trainers.dart`. **Profile card:** the monogram is built from the **name** ("AK" for Anna Kovacs — the old avatar drew the first letter of the e-mail), the picture when there is one, name and e-mail, the gold **"Pro"** chip for a resolved Pro account, and a camera badge (tap = the photo actions, now a `LifeySheet`). **Preferences:** Units and Theme as small primary-on-control pill segments that scale down instead of wrapping at 130 % / Hungarian, Language "System ›" opening a sheet. **Daily goals:** "DAILY GOALS" with **Edit** (opens Body & goals, which recalculates the goals from the body details — a decision: the canvas gives no other target) and **six tiles**, three to a row, each a metric dot + name + the value at 22 px (steps shows the default 10 000 instead of "—"); every tile still opens its own editor sheet. **Workout / Integrations / Account** are grouped `ListGroup` cards: rest timer switch and duration, watch toggle (only with a paired watch), water sources, **Health with an honest subline** read from the real connection ("Off · weight and steps are logged by hand" / "On · weight and steps are imported automatically"), **Notifications "3 on ›"** (counted from the same state the notification screen edits: `enabledCount`, quiet hours not counted), email, change password, Body & goals and **Log out** last, in the heart colour, opening the R5.6 dialog. My trainers (hidden with none) and the subscription tile are on the same rows; the subscription tile's text and chevron are on tokens (the tile test now looks for the rounded chevron). `notification_settings_screen.dart`: `AppBar` → `LifeySubpageHeader`, every `SwitchListTile` → a grouped `SettingsRow` with the same handlers, quiet-hour times as rows. **Every setting that was reachable still is** — the list, checked by a test: units, theme, language, the six goals, rest timer + duration, watch, water sources, Health, notifications (all eight toggles), email, change password, Body & goals, log out, my trainers, subscription/restore, avatar (take / choose / remove), debug gallery in debug builds. ARB: `settingsGoalsEditAction`, `settingsProChip`, `settingsHealthOn/OffSubtitle`, `settingsNotificationsOnCount`, `settingsNotificationsAllOff`. Tests: `settings_screen_test` (title + back, monogram, Pro chip, six tiles with the default step goal, every setting present, Log out → dialog, Health / Notifications rows, notification screen, and 360 dp end-to-end scrolls at ×1.3 in EN / HU, dark and light with no overflow). The Health row is hidden on the desktop test host (no health store), so its screen-level presence is checked on the emulator.
...
*As built:* the derived chat screens are on the same system. **Conversation list** (`conversation_list_screen.dart`): the floating `AdaptiveAppBar` is gone — a large-title `LifeyHeader` with the back button (`onBack`), the search action as a `HeaderIconButton` (still only from 10 rows up) that opens a filled search field under the title, one **grouped card of rows** (`GroupedListItem`, lazily built, pull-to-refresh below the header), an empty state in a bounded `SliverFillRemaining`, a card-coloured skeleton. **`ConversationTile`**: 48 dp tinted avatar, name 16/700 (800 when unread), preview 15/500, time and the 10 dp unread dot on the right (primary when unread, tertiary text otherwise), role / archived labels as `TintedChip`s in a **`Wrap`** so at large text they drop to a second line instead of running out of the row (a new layout test caught the overflow), the mute glyph; an **archived** thread is drawn in the quieter text tiers instead of at 60 % opacity (alpha text fails contrast, `1c252fd`). **In-thread search** (`chat_search_screen.dart`): the `AppBar` is a subpage-header-style row — round back button, a filled field, a clear button — and the hits are `LifeyCard`s with the match highlight unchanged. **New conversation sheet**: a `showLifeySheet` (title, close button) with the explanation, a filled search field and the clients as one `ListGroup` of rows. **Full-screen picture**: the `AppBar` is replaced by a floating `HeaderIconButton` close on the black page. **Paywall**: tokens only, as agreed (its layout belongs to docs/landing_page/69) — the hand-rolled radii are `AppRadius.cardAll` / `AppRadius.pill`. **My trainers** exists only as the Settings section (done in R5.7) and the **trainer invite** card already used tokens with no literals, so nothing changed there. **After this step no plain `AppBar` or `AdaptiveAppBar` is left in `auth`, `onboarding`, `chat`, `settings`, `my_trainers`, `trainer_invite` or `subscription`** (the trainer's own screens are R6). Tests: `conversation_list_screen_test` (search icon updated; +4 layout tests: a mixed list with unread, muted and archived rows at 360 dp × 1.3 in EN / HU, dark and light).
...
*As built:* `GET /trainer/clients` (`TrainerClientResponse`) gains **`avgCalories7d`** and **`prCount7d`**, both nullable read-only integers, no migration. **`avgCalories7d`** — the mean of the daily calorie totals over the last 7 days *of the client's own time zone* (their `utcOffsetMinutes`, like the weekly report), counting only days with something logged (`MealRepository.sumCaloriesBetween`, which already skips soft-deleted meals), rounded; **null when nothing was logged**. **`prCount7d`** — the strength personal records the client set in the last 7 days, derived exactly as the phone derives them (`PersonalRecordCounter`, a pure class): the history is replayed oldest to newest, a set breaks a record when it is strictly better than everything before it in heaviest weight, estimated 1RM (Epley) or reps at the same weight, a kind never fires without an earlier value to beat (a first set is not a record), the sets of one exercise in one session are judged against the running best and each kind counts once per exercise and session; **cardio sessions are not counted** (the card's chip is a strength trophy). Records are not stored anywhere — the counter reads `WorkoutSessionRepository.findFinishedStrengthSetsOldestFirst` (finished, non-deleted strength sessions, oldest first, sets in performed order). 0 is a real answer, null only means it could not be computed. Cost: one query per client for its set history plus seven small sums, on a list that already runs six queries per client — fine for a trainer's client list; if a trainer ever has hundreds of clients this is the place to batch or cache. Tests: `PersonalRecordCounterTest` (9: first set, heavier, equal, reps, three in a row = one, per exercise, records before the window raise the bar but are not counted, bodyweight, empty), `FinishedStrengthSetsQueryRepositoryTest` (3, real Postgres: ordering, and cardio / unfinished / deleted / other users excluded), `TrainerClientControllerTest` (+2: with the figures; without meals and zero records — `avgCalories7d` absent, `prCount7d` 0). All `com.lifey.trainer` suites still pass.
...
*As built:* `TrainerClient` (`trainer/clients/domain/trainer_client.dart`) reads **`avgCalories7d`** and **`prCount7d`** as nullable ints — absent (an older backend), `null` and a decimal-typed number all parse; **null means "no figure", never 0**, and a real `prCount7d` of 0 stays 0 (it simply draws no chip). New derived getter **`weightChangeKg`** — the last minus the first weigh-in of the card's sparkline window, null with fewer than two points (one reading is not a trend) — so the "Weight Δ" KPI always agrees with the line drawn beside it and needs no field of its own. Nothing else about the model, the repository or the online-first, never-persisted rule changed. Tests: `trainer_client_test` (7: the figures read / absent / explicit null with a real 0 / decimal, and `weightChangeKg` for a loss, a gain and one or no reading).
...
*As built:* **the screen** (`trainer_clients_screen.dart`) is a `CustomScrollView` under the large-title `LifeyHeader` — the clay **TRAINER** mark above "My clients" (new `badge` / `badgeHeight` option of the header: it folds away with the scroll like an overline and grows with the text size), add-person, chat (`showDot` when something is unread) and the view-switch avatar as the header's actions — with pull-to-refresh under it and the offline / empty / error / loading states in bounded `SliverFillRemaining`s. Sorting, the "needs attention" block (now `SectionLabel`s) and the two-pane selection are unchanged; the list is lazy. **Sort pills** (`ClientSortChips`): the chosen one filled in the primary colour, the rest card + hairline, scrolling sideways. **The card** (`ClientCard`, `LifeyCard`): a 52 dp monogram tinted per person (the same hue seed as their chat avatar), the name 20/800, a **status line with a dot** — "Active today" / "Active yesterday" in the improvement green, "Last seen 4 days ago" in the warning orange once the compliance threshold (3 days) has passed, "No activity yet" in grey — the weight sparkline at the top right in the weight blue, up to **three KPI tiles** on the nested surface — **Avg kcal** (`avgCalories7d`), **Workouts "6 / wk"**, **Weight "−1.4 kg"** (`weightChangeKg`, in the weight blue) — and the chips under them. **A KPI with no figure is hidden, not drawn as zero** (no meals → no Avg kcal; one weigh-in → no Weight; 2 tiles just share the row); the workouts tile is always there. **Chips** (`ComplianceBadges` on `TintedChip`): "Missed 2 sessions" in the calorie tint, "Weigh-in due" in the weight blue, and the gold **"🏆 2 PRs this week"** (`RecordChip`; 0 or unknown draws nothing). A quiet client is deliberately *not* a chip any more — the status line says it — so the card is not louder than it needs to be. The selected card of the tablet pane keeps its primary ring. ARB: `trainerClientActiveYesterdayLabel`, `trainerClientLastSeenLabel`, `trainerClientKpiAvgKcal/Workouts/PerWeek/Weight`, `trainerClientChipMissed/WeighInDue/Prs`. Tests: `client_card_test` (11: everything present, KPIs hidden not zero, 0 / 1 PR, last seen + warning dot without a "4d" chip, yesterday, missed + weigh-in chips, never logged, no overflow at 360 dp × 1.3 in EN / HU, dark and light); `trainer_clients_screen_test` adjusted (uppercase section labels, a tall view for the lazy list, the sort pill scrolled into view).
...
*As built:* **the header** (`ClientDetailHeader`, now a `PreferredSizeWidget` around `LifeySubpageHeader`, used as `Scaffold.appBar`): round back button, the client's 48 dp monogram (their photo where there is one), the name over "Client since 24 Sep 2026" (new `LifeyFormat.fullDate` — "24 Sep 2026" / "2026. szept. 24."), and a round ⋮ menu; the email line and the old surface card are gone. The states without a client yet (loading, error, "not your client") get the same header bare, so there is a way back from them too. **The tab row** (`ClientTabBar`): text-only 14/700 labels, the active one in the primary text colour with a 3 px primary underline as wide as its tab, a hairline under the row, scrolling from the start edge — the underline follows taps *and* swipes on the body; labels are never shortened (HU "Táplálkozás" / "Ütemterv" scroll into view). The tab order is the tablet canvas's (Overview · Statistics · Workouts · Nutrition · Steps · Weight · Schedule). **The actions** (`ClientActionBar`) sit right under the row on every tab: "Message" (the theme's outlined button — nested surface + hairline) and "Schedule" (primary), captions that shrink instead of ending in "…". Schedule opens the create-schedule sheet from anywhere and refreshes the schedule providers afterwards, so the schedule tab's floating "+" is gone; Message shares one "opening the chat" state with the ⋮ menu's item (both give way to a spinner / disable while it opens). **The overview** (`ClientOverviewTab`): a "Last 7 days" line and four `MetricTile`s in two equal-height rows — **Avg calories** (kcal, "80% of goal" when the client has a calorie goal), **Workouts** ("2 sessions missed · 14 d" from the list summary's missed count, nothing when none), **Avg steps** ("over 5 days"), **Avg RPE** (mean of the RPEs given in the last 7 days from the shared sessions controller, "7.2 / 10" and the effort in words: light / moderate / hard, not maximal / very hard / maximal effort). A figure that does not exist is "—" with no line, never a zero; the goal and the sessions load beside the overview instead of blocking it. Each tile is a door to the tab that explains it. **The weight trend** (`TrendChartCard`, now on `LifeyCard`): title, "Latest: 71.4 kg · Sep 24" under it, the "−1.4 kg · 30 d" change chip in the weight blue when two readings fall in the 30-day window, and the chart in the v2 style (gradient fill, no point markers, the latest reading highlighted) — which the steps / weight / statistics tabs inherit. *Not in the canvas's data:* the goal-relative line for steps ("84% of goal") — the trainer's API has no step goal — so that tile says how many days the average covers; and "all planned done" — there is no planned count — so the workouts tile only speaks up when sessions were missed. ARB: `trainerClientFallbackTitle`, `trainerMessageClientButton`, `trainerOverviewLast7DaysLabel`, `trainerKpi*` (AvgCalories, Workouts, AvgSteps, AvgRpe, RpeScale, CalorieGoal, WorkoutsMissed, StepsDays, RpeLight/Moderate/Hard/VeryHard/Maximal), `trainerWeightChangeChip`, `trainerWeightLatestLabel`; removed `trainerMetric*Label` ×4, `trainerOverviewPeriodHint`, `trainerKgDeltaValue`. Tests: `client_detail_screen_test` (header and since-date, buttons under the tabs on every tab, the underline follows taps and swipes, HU tab labels whole at 360 dp × 1.3, KPI sublines with a goal / missed / rated sessions, dashes instead of zeros, tile → tab, no overflow in EN/HU × dark/light at × 1.3); `client_schedule_tab_test` opens the sheet through the action bar; `lifey_format_test` (+`fullDate`).
...
*As built:* no trainer canvas draws these tabs, so they are the R3/R4/R2 vocabulary applied to the trainer's data — nothing new was invented. **Shared pieces:** `HistoryCard` is now a caps heading over a `ListGroup` (`HistoryRow` became a class: date under a `MetricValue`, unit, optional trailing chip); `ReadOnlyBadge` is a neutral pill (nested surface + hairline, secondary text — a fact about the screen, not a status colour); `KpiGrid` (two tiles a row, equal height, an odd last tile takes half a row) is shared by the overview and statistics; `TrendChartCard` (from R6.5) carries the v2 chart to steps / weight / statistics. **Workouts:** `SessionCard` is a `LifeyCard` with a `ListIconHolder`, the title over the summary line, the date at the right (`LifeyFormat.shortDate`), and `TintedChip`s — RPE in the heart colour, "Client note" neutral, "Your comment" in the primary colour; `sessionTitle()` names a session once for the card and its sheet. **Sheets** (session detail, trainer comment, nutrition goals) moved to `showLifeySheet` — the 20/800 title, handle, keyboard inset and scrolling come from the design system; the exercise / feedback / comment blocks are `LifeyCard.nested`, inputs use the theme's field style (the hard-coded `OutlineInputBorder`s and the literal "kcal" / "g" suffixes are gone). `SessionDetailSheet.show` takes the session (its title needs it). **Nutrition:** day navigator as the segmented control's pill, the totals card a `LifeyCard` with `MetricBar`s, the meal log under a `SectionLabel`, meal-type icons and tints from R2's `mealTypeStyle`, meals as `LifeyCard`s. **Weight / steps:** history rows read like the client's own — weight shows the change since the reading before it (`DeltaChip.signed`), steps the number over its date. **Statistics:** the window chips became `LifeySegmented`, the three figures `MetricTile`s. **Schedule:** rows are `LifeyCard`s with icon holders; "coming up" is one `ListGroup` of `ListRow`s; section headings are `SectionLabel`s; the floating "+" left in R6.5. Deleted: `MetricCard`. Found by the new ×1.3 matrix and fixed: the daily-goals value ("2 000 kcal / 2 100 kcal") and the meal-type header overflowed at 130 % on 360 dp. Tests: `client_detail_screen_test` (weight history deltas, read-only badge, the statistics segmented control, every data tab at 360 dp × 1.3 in EN / HU × dark / light), `client_schedule_tab_test` (the same matrix for the schedule tab); section labels are asserted in caps.
...
*As built:* the breakpoint stays where it was (`isTrainerTwoPane`, 900 dp, in `trainer_layout.dart`); what changed is what the three columns hold. **Rail** (`TrainerNavRail`, 96 dp, in `TrainerShell`): from 900 dp the floating bottom bar gives way to a rail — the brand "L" tile, then one item per destination (a 56 × 36 pill with the icon, the label under it at 11/700; the active pill filled in the primary colour with the filled icon), hairline on its right edge, scrolls instead of overflowing on a short window, each item a semantic button with its selected state; the phone bar is untouched. **List pane** (400 dp, was 380): the same large-title header (clay TRAINER mark, add-person, chat, view switch — the canvas draws only add-person, but the other two are how the trainer reaches the chat and their own log, so they stay), a **"Search clients"** field (`clientSearchControllerProvider`, name or email, case- and accent-insensitive — "szabo" finds "Szabó"; not persisted, tablet only), the sort pills, and **compact rows** (`ClientListRow`: 44 dp monogram, name 15/700, the status line with its dot, chevron; the picked client on the nested surface inside a 1.5 px primary ring) instead of the phone's full cards — the detail beside the list carries the numbers. "Needs attention" still splits the list. The status text moved into `clientStatusOf`, shared with `ClientCard`; on a row an otherwise active client who is overdue for a weigh-in reads "Weigh-in due" in the weight blue, as the canvas's Gábor does. **Detail pane:** wide (≥ 640 dp — a 900 dp window leaves only ~400, which correctly keeps the phone layout inside the pane) it becomes the canvas's layout: `ClientDetailWideHeader` (56 dp monogram, 26/800 name, "Client since …", **Message and Schedule in the header** — `ClientActionButtons`, 140 / 150 dp — so no action bar under the tabs, no back button, no ⋮), the tab row lined up with the header's 32 dp margin, the overview at full width with **four KPIs in one row** and the **weight trend beside an "Upcoming" card** (`UpcomingCard`: up to four rows — 40 dp icon holder, workout, day and time — over hairlines; tap opens the schedule tab), and the list-shaped tabs held to a 720 dp column left-aligned under the header. Selecting a client updates the pane in place (no navigation). Not in the canvas's data: "Goal: build muscle" in the header (no goal is stored) and "· weekly" on the upcoming rows (the row shows day and time). Tests: `trainer_tablet_layout_test` (400 dp pane, compact rows and statuses, search incl. accents / no match / clear, selection fills the pane, header buttons, one row of four KPIs, trend beside Upcoming, the narrow-pane fallback, 1184 × 800 at × 1.3 in EN / HU × dark / light), `trainer_nav_rail_test`; `trainer_clients_two_pane_test` follows the compact row.
...
*As built:* the last trainer screens are on the same system, and **no `AppBar` or `AdaptiveAppBar` is left in `features/trainer`**. **Calendar** (`calendar_screen.dart`): a large-title `LifeyHeader` under the clay TRAINER mark — the month/agenda toggle, the client filter (a dot when it is narrowing the agenda), chat (dot when unread) and the view switch as round header buttons, the month name as the title in the month view; "1 client ✕" under the header is a `TintedChip` in the primary colour; the week strip is a `LifeyCard` (32 dp day circles — filled primary when selected, a ring for today — and up to three status dots) and the agenda one `LifeyCard` per session (time, 36 dp monogram, name over workout, status chip); the month grid sits in a card, its cell aspect ratio growing with the text size (the layout matrix found a 2 px overflow at × 1.3); the client filter and the session peek are `showLifeySheet`s (`ListGroup` of clients with checkboxes; the peek's title is the client, its facts a `LifeyCard.nested`), the create-schedule sheet too — repeat as `LifeySegmented`, start / until / time as `ListRow`s, the summary a nested card (the warning tint when there is a problem). **`OccurrenceStatusChip`** is a `TintedChip`, and `occurrenceStatusColor` is the one colour language for it, the strip's dots and the month grid: upcoming primary, done improvement green, missed the warning orange, cancelled grey. **Assignments**: header + `FilterPillRow` (extracted from `ClientSortChips`, shared with the client sort pills) for the type and client filters, an assignment a `LifeyCard` with a `ListIconHolder` (workout in the primary colour, recipe in protein green), the assign sheet (two steps in a fixed-height `showLifeySheet`: content as a `ListGroup`, clients with checkboxes — those who already hold it locked and ticked) and the result sheet. **Programs**: header, program cards (`LifeyCard` + the active-count `TintedChip`, the picked one ringed in the tablet's pane), the "edit on the web" notice a `NoticeCard` in the clay accent; `program_detail_screen` uses the **subpage header** (no back button in the tablet's pane), each week a `LifeyCard` around the expansion tile with nested cards for the slots; the "start program" sheet as the assign sheet. **Invites**: subpage header, the theme's field, a 56 dp send button, a `SectionLabel` and one `ListGroup` of pending invites. **Settings**: subpage header and one `ListGroup` — the invites row and the weekly-report row with a real `Switch` (tests follow: `SwitchListTile` → `Switch`). Wide screens keep these single-column screens to `trainerContentMaxWidth`; on a tablet the FABs no longer reserve the phone bar's slot (`TrainerFabPadding` / `trainerFabBottom` check `isTrainerTwoPane`). Tests: a layout matrix (360 dp × 1.3 in EN / HU × dark / light) for the agenda, the month view, the program library, a program's weeks, the invites and the settings; new `assignments_screen_test` (mark, cards, type pills, the two-step sheet with a locked holder, and the matrix).
...
*As built:* what was left was not a list of undesigned screens but the inner painting of the R3 cardio screens and a long tail of one-off literals, so R7.2 was done by the audit's own categories rather than file by file. **Audit refinement:** `BorderRadius.circular(AppRadius.card)` is the token in use, so the audit now counts only the calls that take a number (the baseline of that column falls from 142 to 80 without touching a line; the whole baseline is **410** — `fontSize` 322, `Color(0x` 7, `BorderRadius.circular` 80, `AdaptiveAppBar` 1). **Radii:** the 28 legacy `AppRadius` aliases were renamed to their v2 names (same values); every numeric `BorderRadius.circular(n)` became the nearest step (≤ 8 tag, ≤ 16 control, ≤ 24 card, larger hero, 999 pill; the two conditional ones `nested ? 14 : 18` / `auto ? 22 : 20` too). **Type:** 233 raw `TextStyle(fontSize: …)` constructors became theme roles by size (≤ 12.5 → `labelSmall`, 13 → `bodySmall`, 14 → `titleSmall`, 15 → `bodyMedium`, 16–17 → `titleMedium`, 18–21 → `titleLarge`, 22–24 → `headlineSmall`, 26–30 → `headlineMedium`, 34 → `headlineLarge`) or, for the hero numbers (≥ 40, and the tabular / tightly tracked 20–34 ones), `AppType.number(size)`; another 68 `textTheme.x.copyWith(fontSize: n)` overrides were re-based on the slot nearest to n, keeping the weight the old slot gave. The design's smallest type is now 12 (the cardio screens had 9–11.5 px labels), so the cardio, interval-plan and music sheets were checked for overflow by the existing ×1.3 tests. `AppType.unit` takes an explicit `size` for the two units the canvas sets off the 42 % rule. **Left on purpose, marked `// design-audit: ok`:** the monogram / glyph sizes proportional to their disc (`chat_avatar`, `client_avatar`, `provider_glyph`), the thumbnail radius `size / 6`, and the two painter labels with no `BuildContext` (`route_painter`, `pace_bar_chart`). **Colour:** the last `Color(0x…)` is `AppElevation.pillThumbDark`, the dark thumb shadow shared by `LifeySegmented` and `PillTabBar`. **`confirm_delete_dialog` was the worst offender in substance, not just in count:** a hard-coded dark dialog (its own background, title and scrim colours) that stayed dark in the light theme on all 17 call sites; it is now the canvas confirm dialog on the dialog theme — the 48 dp heart-tinted holder (or the accent's), the theme's title and body, Cancel / Delete stacking above 115 % text — with a light / dark / HU × 1.3 test. Audit after R7.2: **6, all in `adaptive_app_bar.dart` (deleted in R7.3)**, 5 more allowed by marker. Not counted by the audit and left as they are: 40 `showModalBottomSheet` calls (they take the themed sheet, and most hold content `showLifeySheet`'s scroll frame cannot host — noted for `REMAINING-WORK.md`), and the `ColorScheme` role accessors (`surfaceContainer`, `onSurfaceVariant`), which R0.1 mapped onto the v2 palette.
...
### R6 — 2026-09-26 — Pixel_10 AVD (1080 × 2424, 420 dpi = 411 × 923 dp, API 37) and the same AVD as a **1280 × 800 dp tablet** (`wm size 800x1280`, `wm density 160`, landscape)

Scope: the trainer family against `Lifey 6 Trainer.dc.html` — 9.1 My clients, 9.2 Client overview, the Trainer tablet frame — on a local backend with a seeded trainer (`mark.r6@lifey.test`) and four clients (Anna: 6 workouts at RPE 7.2, 1 631 kcal against a 2 360 goal, 66.0 → 64.6 kg, 6 PRs, three weekly schedules; Bence quiet for 3 days; Eszter on a program; Gábor overdue for a weigh-in), a pending invite and a program.
Walked: the clients list, the client detail (Overview, Statistics, Workouts, Nutrition, Weight, Schedule), calendar (agenda), assigned, programs and a program's weeks, the avatar menu, trainer settings, invites, the logout / login round trip and the "Trainer view" switch with its intro sheet. Matrix: dark EN (all of the above, phone and tablet), light HU (list, client detail, schedule tab), light HU × 1.3 (overview, list), reduced motion (client detail). Canvas frames rendered with Playwright and put next to emulator screenshots (sent). The backend was restarted on the current code first — the one that had been running predated R6.2 and returned no `avgCalories7d` / `prCount7d`.

**Matches**
- My clients: the clay TRAINER mark over "My clients", add-person / chat / avatar in round 44 dp buttons, the sort pills (chosen one primary), "NEEDS ATTENTION" / "ALL CLIENTS" labels, cards with the tinted monogram, the status line with its dot (green "Active today", orange "Last seen 3 days ago"), the weight sparkline, three KPI tiles (Avg kcal · Workouts / wk · Weight change in the weight blue), the gold "🏆 6 PRs this week" chip, "Weigh-in due" in the weight blue. A KPI without a figure is hidden.
- Client overview: the subpage header (round back, 48 dp monogram, name over "Client since …", ⋮), the underlined tab row with the active underline (the row scrolls; the tab you left on is remembered), Message (outlined) and Schedule (primary) right under it, "Last 7 days", four tiles with their context lines ("69% of goal", "over 7 days", "7.2 / 10 hard, not maximal"), and the weight trend with the "−1.4 kg · 30 d" chip, the latest reading and the min / mid / max axis. Light and Hungarian ("Áttekintő", "Ütemterv", "Cél: 69%", "Klienseid 2026. márc. 10. óta") and ×1.3 read the same, with nothing truncated in the tiles.
- Tablet: rail 96 · list 400 · detail flex; "Search clients", compact rows with the picked client ringed; the header with Message / Schedule; four KPIs in one row; the weight trend beside "Upcoming"; selecting a client updates the pane in place.
- The other tabs and screens (no canvas — checked against the family): Workouts as cards with RPE / Note chips, Nutrition (goal bars, meal groups), Weight history with deltas, Schedule (series cards, "Coming up" list), Calendar (week strip card, agenda cards, status chips), Assigned (filter pills, cards), Programs (cards, the clay "built on the web" notice, weeks as expandable cards), Trainer settings and Invites (grouped rows, real switch) — all with subpage headers, none with an `AppBar`.

**Deviations (intended)**
- Steps tile says "over 7 days" and Workouts speaks only when sessions were missed: the trainer API has neither a step goal nor a planned-session count — R6.5. "Goal: build muscle" is absent from the header (no goal is stored) and "· weekly" from the upcoming rows — R6.7.
- The tablet header keeps chat and the view switch beside add-person (the canvas draws only add-person) — R6.7. Message is also in the ⋮ menu, which holds nothing else — R6.5.
- Sparklines in the seeded data are zig-zags because the seed adds ±0.05 kg of noise; the card's weight change is computed from the summary's trend points (last − first), not over a fixed 30 days.

**Bugs (fixed on `feature/mobile-redesign`, R6.fix-1…4)**
- The view-switch avatar in every trainer header was a 30 dp circle among 44 dp buttons; the bottom bar named the first tab "My clients" (canvas: "Clients") and Programs used a different glyph than the canvas → **R6.fix-1** (`MonogramAvatar` at 44 dp, `trainerNavClientsLabel`, `view_list`).
- The floating bar stayed on top of a client's tabs and covered the last rows of every list (the canvas overview has none) → **R6.fix-2** (the client route is pushed on the root navigator).
- The weight chart had no value axis (canvas: min / max labels) → **R6.fix-3** (`axisLabelBuilder` on the trend card: weight, steps).

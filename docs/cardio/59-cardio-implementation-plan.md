...
| AW 21 / W 20 | Összegzés + szinkron | C5.7a, C5.7b |
...
## 9. C5 — Óra (7 lépés, C5.7 a/b-re bontva) · MF5

**C5.1–C5.3 tiszta Dart, telefon-oldali** ([55 §7](55-cardio-watch-plan.md) W-1–W-4) — a
`standalone_session_processor`/`watch_session_merge`/`watch_set_log_decision` cardio-ága, az
indító payload bővítése, a `rankQuickStartEntries()`-re épülő egyesített picker payload: mind a
meglévő `mobile/lib/core/watch/` Dart-hídban élnek, nincs bennük natív kód — ugyanúgy
**Windowson fejleszthető és tesztelhető**, mint a C1–C3 eddigi lépései.

**C5.4/C5.5 tisztán watchOS** (W-5, Swift, `mobile/ios/LifeyWatch/`) — az aktivitástípus-térkép és
a három cardio-elrendezés SwiftUI-ban. Xcode nélkül (típusellenőrzés, Preview, szimulátor nélkül)
ezt blind szövegszerkesztésként írni túl kockázatos ahhoz, hogy értelmesen "fejleszthetőnek"
számítson — ellentétben C4a.1 Info.plist-szerkesztésével, ez valódi, logikát tartalmazó UI-kód.
**Mac kell.**

**C5.6 tisztán Wear OS** (W-6, Kotlin, Health Services `ExerciseClient`) — a `mobile/android`
watch-modul aktivitástípus/`dataTypes`-térképe és ugyanazok a képernyők Compose for Wear OS-ben.
Az Android-eszközlánc Windowson is teljes (ugyanaz az elv, mint a telefon-oldali natív Android
munkánál, pl. C2.9–C2.11a, C4a.5a) — **Windowson fejleszthető**.

**C5.7 ([55 §7](55-cardio-watch-plan.md) W-7 + W-8 + W-9) három dolgot fésül össze** — zárás-
összegzés bővítés (zónák, táv, szintemelkedés), standalone cardio indítás, GAME pályán/padon
kétirányú szinkron —, és **mindegyik mindkét platform natív oldalát érinti**, plusz egy közös
Dart-felet a telefonon (a forrás-jelölt beírás, ami platformfüggetlenül ugyanaz, akármelyik óra
küldte). Ezért, a **C4a.5 mintáját** követve, **a/b lépésre bontva**: a **C5.7a** ág a Wear OS
natív munka + a platformfüggetlen Dart-fél, Windowson kész; a **C5.7b** ág a watchOS natív munka,
**Mac-et igényel**.
...
elv, mint a C4a.5a/b utáni G11 akku-mérésnél: egyik platform végpróbája sem futtatható a másikon,
és egyik sem Windowson/Mac-en közvetlenül — mindkettő saját fizikai/emulált eszközt igényel.

...
public interface ChatConversationRepository extends JpaRepository<ChatConversation, Long> {
     ...
     * has their own trainer sees both in one list (§6.1). The peers' display
     * names are resolved separately, in one batch, through
     * {@code ChatUserDirectory} — the rows themselves only carry ids.
     */
     ...
     */
    @Query("select c from ChatConversation c "
            + "where c.id = :id and (c.trainerId = :userId or c.clientId = :userId)")

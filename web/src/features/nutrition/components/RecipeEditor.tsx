...
export function RecipeEditor({ recipe, onSaved, onCancel }: RecipeEditorProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      show(recipe ? "Recipe updated" : "Recipe created", "success");
      onSaved();
    },
    onError: () => show("Failed to save recipe", "error"),
  });
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg rounded-[var(--r-lg)] p-5 flex flex-col gap-4 max-h-[90vh] overflow-y-auto"
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-base">{recipe ? "Edit recipe" : "New recipe"}</h3>
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs font-semibold" style={{ color: "var(--on-surface-variant)" }}>Ingredients</p>
          {ingredients.map((ing, idx) => (
            <div key={idx} className="flex items-center gap-2 px-3 py-2 rounded-[var(--r-md)]"
              ...
              <button onClick={() => setIngredients((prev) => prev.filter((_, i) => i !== idx))}
                style={{ color: "var(--muted)" }} aria-label="Remove ingredient">
          ...
          <div className="flex items-center gap-2 px-3 h-9 rounded-[var(--r-md)]"
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Add ingredient…"

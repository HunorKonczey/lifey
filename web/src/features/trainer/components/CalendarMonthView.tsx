...
export function CalendarMonthView({ monthAnchor, sessions, onSelectDay, onSelectSession }: CalendarMonthViewProps) {
  ...
  const weekdayLabels = Array.from({ length: 7 }, (_, i) =>
    format(days[i], "EEEE", { locale: dateLocale }).toUpperCase(),
  );
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              className="rounded-xl p-1.5 flex flex-col gap-1 min-w-0 text-left cursor-pointer"
              style={{
            ...
            >
              {dayIsToday ? (
                ...
                >
                  {format(day, "d")} · {t("today").toUpperCase()}
                </span>
              ) : (
                <span

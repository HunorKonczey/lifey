...
class SettingsControllerTest {
    ...
    void update_withHungarianReturnsIt() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/settings").contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"unitSystem":"METRIC","theme":"SYSTEM","language":"HUNGARIAN","workoutReminderEnabled":true,"trainerCommentPushEnabled":true,"trainerGoalsPushEnabled":true}
                                """))
                .andExpect(status().isOk())
    ...
    void update_withDailyStepGoalReturnsIt() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/settings").contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"unitSystem":"METRIC","theme":"SYSTEM","language":"SYSTEM","dailyStepGoal":10000,"workoutReminderEnabled":true,"trainerCommentPushEnabled":true,"trainerGoalsPushEnabled":true}
                                """))
                .andExpect(status().isOk())

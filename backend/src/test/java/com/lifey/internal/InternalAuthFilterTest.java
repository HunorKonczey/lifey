 ...
 * {@code POST /internal/push} notifies an arbitrary user id — so this filter is
 * the only thing between the internet and a notification spammer
...
class InternalAuthFilterTest {
    ...
    void aMissingHeaderIsRejected() throws Exception {
        ...
        filter(SECRET).doFilter(new MockHttpServletRequest("POST", "/internal/push"), response, chain);
    ...
    void withNoSecretConfiguredEverythingIsRejected() throws Exception {
        ...
        // deployment that forgot the variable should lose push (loud, fixable),
        // not expose an unauthenticated way to notify any user.
    ...
    private static InternalAuthFilter filter(String token) {
        return new InternalAuthFilter(
                new InternalApiProperties(token, "", Duration.ofSeconds(2), Duration.ofSeconds(5)),
    ...
    private static MockHttpServletRequest request(String presentedToken) {
        ...
        request.addHeader(InternalAuthFilter.TOKEN_HEADER, presentedToken);
        return request;
    }
}
